...
if_anon_info_msg = """
Вы хотите оставить запрос или предложение о сотрудничестве анонимно или персонально? Если вы хотите оставить свои контакты, требуется согласие на обработку персональных данных. Ознакомиться можно [здесь](http://quarteera.de)."""
...
teams_dic = {
    "team_gen": "комьюнити-направление: работа организации",
    "team_chat": "комьюнити-направление: модерация чатов",
    "team_event": "комьюнити-направление: проведение мероприятий",
    "team_cont": "комьюнити - направление: предложение по контенту",
    "team_cons": "консультационная помощь (юридическая / психологическая)",
    "team_reloc": "команда по помощи в релокации",
    "other": "другое"}
...
def callback_inline(call):
    if call.message:
        if call.data == "butt_yes" or "butt_no":
            ...
            keyboard = types.InlineKeyboardMarkup()
            team_gen = types.InlineKeyboardButton(text="комьюнити-направление: работа организации", callback_data="team_gen")
            team_chat = types.InlineKeyboardButton(text="комьюнити-направление: модерация чатов", callback_data="team_chat")
            team_reloc = types.InlineKeyboardButton(text='помощь в релокации', callback_data="team_reloc")

            keyboard.add(team_gen)
            keyboard.add(team_chat)
            keyboard.add(team_reloc)
        ...
        if call.data in teams:
                team_name = teams_dic[call.data]
                review = bot.send_message(call.message.chat.id,
                                 ...
                                 reply_markup=markup, parse_mode="Markdown")
                #team_name, review into a database

        if call.data == "info_yes" or "info_no":
            if call.data == "info_yes":
                pers_email = bot.send_message(call.message.chat.id, "Оставьте ваш email для обратной связи", reply_markup=markup)
            #check if the format is correct
            #save to db
            info_offer = bot.send_message(call.message.chat.id,
                                          "Чтобы оставить запрос или предложение о сотрудничестве, просто отправь свой отзыв ответным сообщением. Оно будет переправлено этой команде напрямую.",
                                          reply_markup=markup, parse_mode="Markdown")
